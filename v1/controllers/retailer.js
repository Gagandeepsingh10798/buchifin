  ...
  updateById: async (req, res, next) => {
    try {
      ...
      if (!ObjectId.isValid(userId)) {
        return universal.response(res, CODES.BAD_REQUEST, MESSAGES.INVALID_ID, {});
      }


      if (updates.phone) {
        updates.phone.forEach(async phoneData => {
          if (phoneData._id && ObjectId.isValid(phoneData._id)) {
            await Models.User.updateOne(
              { "_id": userId, "phone._id": phoneData._id },
              { $set: { "phone.$": phoneData } }
            );
          } else {
            await Models.User.updateOne(
              { "_id": userId },
              { $push: { phone: phoneData } }
            );
          }
        });
      ...
      if (updates.address) {
        updates.address.forEach(async addressData => {
          if (addressData._id && ObjectId.isValid(addressData._id)) {
            await Models.User.updateOne(
              { "_id": userId, "address._id": addressData._id },
      ...
      const pipeline = [
        { $match: { type: config.get("USER_TYPES.RETAILER_ADMIN"), isDeleted: false, _id: new ObjectId(userId) } },
        {
          $lookup: {
            from: 'users',
            localField: 'createdBy',
